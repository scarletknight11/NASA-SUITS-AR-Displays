...
namespace Microsoft.MixedReality.OpenXR
{
        ...
        : OpenXRFeature, IOpenXRContext, ISubsystemPlugin where TPlugin : OpenXRFeaturePlugin<TPlugin>
    {
        ...
        protected override bool OnInstanceCreate(ulong instance)
        {
            if (Instance != 0)
            {
                Debug.LogWarning("New instance was created without proper destroy the previous one.");
            }

            Instance = instance;

            NativeLib.OnInstanceCreated(nativeLibToken, instance, PFN_xrGetInstanceProcAddr);
        ...
        protected override void OnInstanceDestroy(ulong instance)
        {
            if (Instance == 0)
            {
                Debug.LogWarning("Instance has been destroyed already.");
                return;
            }

            if (SystemId != 0)
            {
                // Unity's OnSystemChange event won't trigger when destroying
                // Reset system before destroying the instance.
                SystemId = 0;
