...
{
    /// <summary>
    /// LinePainter draws poly-line according to the user's hand grips and releases.
    /// </summary>
    public class LinePainter : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            int playerIndex = handOverlayer ? handOverlayer.playerIndex : 0;

            if (kinectManager && kinectManager.IsInitialized() && kinectManager.IsUserDetected(playerIndex))
            {
                if (infoText)
                {
                    infoText.text = "Close your hand to start drawing. Press [U] to undo the last line.";
                }
            }
